                ...
                Column(
                  children: [
                    Container(
                      ...
                      child: Column(
                        // Fixed: Wrap children inside a Column
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text(
                            'Possible effects:',
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                        'Take ${medicine.beforeFood ? "before" : "after"} food'),
                  ],
                ),
              ),
            ))
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final Map<String, dynamic> data =
            jsonDecode(response.body); // Correctly decode JSON
        if (data['adr_detected'] == true) {
          // Correctly access map values
          setState(() {
            _effects =
                (data['potential_adverse_effects'] as List<dynamic>).join(", ");
            _recommendation = data['recommendation'];
          });
        }
      } else {
        throw Exception(
            'Failed to predict ADR. Status code: ${response.statusCode}');
      }
    } catch (e) {
